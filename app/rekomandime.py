...
def rekomandime(request):
    ka_vleresim = Vlersim.objects.filter(perdorues=request.user).exists()
    if not ka_vleresim:
        return render(request, 'libratemi.html', {})
    ...
    liberat_df = pd.DataFrame.from_records(liber_queryset, columns=kolonat_e_librit)
    ...
    perdoruesi_loguar = request.user.id
    perdoruesit_e_ngjashem = perdorues_te_ngjashem(perdoruesi_loguar, vlersim_matrix)
    libra_te_rekomaduar = rekomando_liber(perdoruesi_loguar, perdoruesit_e_ngjashem, vlersim_matrix, liberat_df)
    data = {
        'titulli_i_faqes': 'Librat e Sygjeruar',
        'librat': [lib.as_dict() for lib in Liber.objects.filter(iid__in=libra_te_rekomaduar)],
        'edit': False
    }
